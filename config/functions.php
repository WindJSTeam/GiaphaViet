<?php
	...
	function deleteperson($id){
		$db = new DB();
		$sql = "SELECT * FROM INFO_PERSON WHERE FATHER_ID = ".$id."";
		$result = $db->Query($sql);
		while($row = mysql_fetch_assoc($result)){
			if(!deleteperson($row["ID"]))	return false;
		}
		$sql = "DELETE FROM INFO_PERSON WHERE ID = ".$id."";
		$result = $db->Query($sql);
		if(!$result){
			return false;
		}
	...
	function youtube_poster($url){
		$frags = explode('/watch?v=',$url);
		return 	'http://i1.ytimg.com/vi/'.$frags[1].'/default.jpg';
	}
	...
	function locdau ( $alias )
		...
		*/
		...
		function idtourl($name,$id){
			return locdau(m_unhtmlchars($name)).'-'.$id.'.html';
		}
